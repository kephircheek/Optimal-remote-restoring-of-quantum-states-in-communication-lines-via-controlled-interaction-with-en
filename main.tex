...
\label{rhonm}
&&
\partial_t\rho_{nm}(t) = \sum_{ij} \tilde T^{(k)}_{nm;ij}\rho_{ij}(t), \;\; t_{k-1} < t \le t_k,\\\nonumber
&&
\tilde T^{(k)}_{nm;ij}= -i (H_{ni} - H_{jm}) + \sum_{l=N-N^{(ER)}}^N a_{lk} ((Z_l)_i (Z_l)_j \delta_{ni}\delta_{mj} - \frac{1}{4} \delta_{ni}\delta_{mj}.
\end{eqnarray}
 Introduce the matrix $\hat T^{(k)} =\{\tilde T^{(k)}_{nm;ij}\}$  where each pair ofindexes $(nm)$ and $(ij)}$ is treated as a single index.
 Then
 \begin{eqnarray}\label{rhoT}
 \vec{\rho}(t) = e^{\hat T^{(k)} t}\vec{\rho}(0),  \;\; t_{k-1} < t \le t_k.
 \end{eqnarray}
