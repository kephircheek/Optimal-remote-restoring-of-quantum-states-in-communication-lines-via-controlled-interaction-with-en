...
% =\sum_{i_Sj_S}W_{(0 n_R)(0m_R);(i_S0)(j_S0)}  \rho^{(S)}_{i_Sj_S}(0) .
\end{eqnarray}
where
\begin{eqnarray}
...
We shall emphasize that the scale parameters $\lambda_{ij}$ are universal, i.e., they do not depend on the elements of  the initial sender's density matrix and are completely determined by the Hamiltonian and the selected time instant for state registration {$t_{\mathrm{reg}}$}.

%Density matrices $\rho^{(R)}(t)$ and $\rho^{(S)}(0)$ are related as
%\begin{eqnarray}\label{rhoR}
%\rho^{(R)}_{nm}(t) = \sum_{i,j=0}^{N^{(S)}-1}T_{nm;ij}(t) \rho^{(S)}_{ij}(0), \;%\;n,m = 0,\dots,2^{N^{(R)}}-1,
%\end{eqnarray}
%where matrix elements of $T$ are expressed in terms of matrix elements of $W$ as
%follows.
...
&&T_{n_Rm_R;i_Sj_S}=\sum_{n_1=0 }^{N-N^{(S)}-1}W_{(n_1n)(n_1m);ij}, \\\nonumber
&&n_R,m_R=0,\dots,2^{N^{(R)}}-1,\;\;i_S,j_S=0,\dots,2^{N^{(S)}}-1.
\end{eqnarray}
%Here we assume the binary representation for the subscripts that allows us to split
%the first pair of indexes in $W$ in two parts (i.e., $nm\to  (n_1n)(n_1 m)$), where %$n_1$  counts (in binary form) the number of states in $S\cup TL$.
The tensor $T$ transfers the initial  density matrix of the sender $S$  to the receiver $R$. Unlike, \cite{}, this operator can not be represented as  a composition of Kraus operators.
 ...
 Next, we express the $\lambda$-parameters in (\ref{rest}) in terms of the elements of $T$:
...
\end{eqnarray}
\fi
The restoring constraints that provide restored structure (\ref{rest}) for $i\neq j $ (nondiagonal elements of $\rho^{(R)}$) at time instant of state registration $t_{\mathrm{reg}}$ read
...
Then
 ...
 \end{eqnarray}
%Here the elements of $\vec{\rho}$ are $\rho_{nm}$, i.e., they are enumerated by
%the pair of indexes
...
and separate evolution under $H$ and under Landblad terms. Thus, we have for the operators $W_j$ in (\ref{UtK}) (hereafter $\tilde n$ is the Trottorization number): %see (\ref{rhoT}),
...
\end{eqnarray}
so that $\gamma_j=0$ over $\Delta\tau^{(2)}$.
...
&&
\min_{i,j} a_{ij} \gg \max_{i,j} D_{ij} \;\;\;{\mbox{over}}\;\;\; \Delta \tau^{(1)}, \\\nonumber
...
%$$
%where $\|\cdot\|$ is some matrix norm. In other words,  large $\|\gamma_j\|$s act
%over the short time interval.
This means that the Landblad term dominates  over $H$ on the interval
$\Delta\tau^{(1)}$.
and we can write the approximate evolution operator $W^{(k)}(\Delta \tau)$ as:
